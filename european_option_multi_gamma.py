# -*- coding: utf-8 -*-
"""
European call option with multiple interest rate curves
...
class FNN(tf.keras.Model):
    ...
    def call(self,inputs):
        t, rt, St, kappa, sigma_r, sigma_S, rho, b0, b10, b11, c1, T = inputs
...
class RNN(tf.keras.Model):
    ...
    def call(self,inputs):
        t, rt, St, kappa, sigma_r, sigma_S, rho, b0, b10, b11, c1, T = inputs
        u0 = tf.concat([t, rt, St, kappa, sigma_r, sigma_S, rho, b0, b10, b11, c1, T], axis=1)
...
class DGM(tf.keras.Model):
    ...
    def call(self, inputs):
        t, rt, St, kappa, sigma_r, sigma_S, rho, b0, b10, b11, c1, T = inputs
        x    = tf.concat([t, rt, St, kappa, sigma_r, sigma_S, rho, b0, b10, b11, c1, T], axis=1)
        S1 = self.Sw(x)
        out = S1
...
def loss_function(model, t,  rt, St, kappa,  sigma_r,  sigma_S,  rho,  b0,  b10,  b11,  c1,  T,
                             rT, ST, kappaT, sigma_rT, sigma_ST, rhoT, b0T, b10T, b11T, c1T, TT, HT,
                         tl, rl, Sl, kappal, sigma_rl, sigma_Sl, rhol, b0l, b10l, b11l, c1l, Tl, Hl,
                         sc, loss_weights):
    ...
    b0l_tf      = tf.convert_to_tensor(b0l, dtype=tf.float32)
    b10l_tf     = tf.convert_to_tensor(b10l, dtype=tf.float32)
    b11l_tf     = tf.convert_to_tensor(b11l, dtype=tf.float32)
    c1l_tf      = tf.convert_to_tensor(c1l, dtype=tf.float32)
    Tl_tf       = tf.convert_to_tensor(Tl, dtype=tf.float32)
    ...
    with tf.GradientTape(persistent=True) as tape1:
        ...
        u_pred = model([t_tf, rt_tf, St_tf, kappa_tf, sigma_r_tf, sigma_S_tf, rho_tf, b0_tf, b10_tf, b11_tf, c1_tf, T_tf])
    ...
    # maturity loss
    u_pred_T   = model([TT_tf, rT_tf, ST_tf, kappaT_tf, sigma_rT_tf, sigma_ST_tf, rhoT_tf, b0T_tf, b10T_tf, b11T_tf, c1T_tf, TT_tf])
    residual_T = u_pred_T - HT_tf
    L_T        = tf.reduce_mean(tf.square(residual_T))
    
    # lower loss
    u_pred_l   = model([tl_tf, rl_tf, Sl_tf, kappal_tf, sigma_rl_tf, sigma_Sl_tf, rhol_tf, b0l_tf, b10l_tf, b11l_tf, c1l_tf, Tl_tf])
...
def train_function(model, t,  rt, St, kappa,  sigma_r,  sigma_S,  rho,  b0,  b10,  b11,  c1,  T,
                              rT, ST, kappaT, sigma_rT, sigma_ST, rhoT, b0T, b10T, b11T, c1T, TT, HT,
                          tl, rl, Sl, kappal, sigma_rl, sigma_Sl, rhol, b0l, b10l, b11l, c1l, Tl, Hl,
                          sc, loss_weights, epochs=10000, lr=0.001, batch_perc=0.20):
    ...
    for epoch in range(epochs):
        ...
        for batch in range(num_batches):
            ...
            b0l_batch      = b0l[batch_indicesl]
            b10l_batch     = b10l[batch_indicesl]
            b11l_batch     = b11l[batch_indicesl]
            c1l_batch      = c1l[batch_indicesl]
            Tl_batch       = Tl[batch_indicesl]
            ...
            with tf.GradientTape() as tape:
                ...
                loss, L_D, L_T, L_l, L_tot = loss_function(model, 
                                                           t_batch,  rt_batch, St_batch, kappa_batch,  sigma_r_batch,  sigma_S_batch,  rho_batch,  b0_batch,  b10_batch,  b11_batch,  c1_batch,  T_batch,
                                                                     rT_batch, ST_batch, kappaT_batch, sigma_rT_batch, sigma_ST_batch, rhoT_batch, b0T_batch, b10T_batch, b11T_batch, c1T_batch, TT_batch, HT_batch,
                                                           tl_batch, rl_batch, Sl_batch, kappal_batch, sigma_rl_batch, sigma_Sl_batch, rhol_batch, b0l_batch, b10l_batch, b11l_batch, c1l_batch, Tl_batch, Hl_batch,
                                                           sc, loss_weights)
...
T_max     =  10                      # maximum maturity consider
# NS parameters
...
Directory_name = "EU_multi_gamma__"+NN_type+"_"+str(num_hidden_layers)+"_"+str(num_neurons)+"__"+str(num_samples)+"_"+str(num_samples_bound)+"__"+str(inner_loss_weight)+"_"+str(maturity_loss_weight)+"_"+str(lower_loss_weight)
...
b0      = np.random.uniform(b0_l,b0_u, num_samples).reshape(-1,1)
b10     = np.random.uniform(b10_l,b10_u, num_samples).reshape(-1,1)
b11     = np.random.uniform(b11_l,b11_u, num_samples).reshape(-1,1)
c1      = np.random.uniform(c1_l,c1_u, num_samples).reshape(-1,1)
...
b0T      = np.random.uniform(b0_l,b0_u,num_samples_bound).reshape(-1, 1)
b10T     = np.random.uniform(b10_l,b10_u,num_samples_bound).reshape(-1, 1)
b11T     = np.random.uniform(b11_l,b11_u,num_samples_bound).reshape(-1, 1)
c1T      = np.random.uniform(c1_l,c1_u,num_samples_bound).reshape(-1, 1)
...
b0l      = np.random.uniform(b0_l,b0_u,num_samples_bound).reshape(-1, 1)
b10l      = np.random.uniform(b10_l,b10_u,num_samples_bound).reshape(-1, 1)
b11l      = np.random.uniform(b11_l,b11_u,num_samples_bound).reshape(-1, 1)
c1l      = np.random.uniform(c1_l,c1_u,num_samples_bound).reshape(-1, 1)
...
if TRAIN:
    ...
    loss_tab_1 = train_function(model, 
                                t_tilde,  rt_tilde, St_tilde, kappa,  sigma_r,  sigma_S,  rho,  b0,  b10,  b11,  c1,  T_tilde,
                                          rT_tilde, ST_tilde, kappaT, sigma_rT, sigma_ST, rhoT, b0T, b10T, b11T, c1T, TT_tilde, HT,
                                tl_tilde, rl_tilde, Sl_tilde, kappal, sigma_rl, sigma_Sl, rhol, b0l, b10l, b11l, c1l, Tl_tilde, Hl,
                                sc, loss_weights, epochs_1, lr_1, batch_perc=0.34)

    loss_tab_2 = train_function(model, 
                                t_tilde,  rt_tilde, St_tilde, kappa,  sigma_r,  sigma_S,  rho,  b0,  b10,  b11,  c1,  T_tilde,
                                          rT_tilde, ST_tilde, kappaT, sigma_rT, sigma_ST, rhoT, b0T, b10T, b11T, c1T, TT_tilde, HT,
                                tl_tilde, rl_tilde, Sl_tilde, kappal, sigma_rl, sigma_Sl, rhol, b0l, b10l, b11l, c1l, Tl_tilde, Hl,
                                sc, loss_weights, epochs_2, lr_2, batch_perc=0.34)

    loss_tab_3 = train_function(model, 
                                t_tilde,  rt_tilde, St_tilde, kappa,  sigma_r,  sigma_S,  rho,  b0,  b10,  b11,  c1,  T_tilde,
                                          rT_tilde, ST_tilde, kappaT, sigma_rT, sigma_ST, rhoT, b0T, b10T, b11T, c1T, TT_tilde, HT,
                                tl_tilde, rl_tilde, Sl_tilde, kappal, sigma_rl, sigma_Sl, rhol, b0l, b10l, b11l, c1l, Tl_tilde, Hl,
                                sc, loss_weights, epochs_3, lr_3, batch_perc=0.34)

    loss_tab_4 = train_function(model, 
                                t_tilde,  rt_tilde, St_tilde, kappa,  sigma_r,  sigma_S,  rho,  b0,  b10,  b11,  c1,  T_tilde,
                                          rT_tilde, ST_tilde, kappaT, sigma_rT, sigma_ST, rhoT, b0T, b10T, b11T, c1T, TT_tilde, HT,
                                tl_tilde, rl_tilde, Sl_tilde, kappal, sigma_rl, sigma_Sl, rhol, b0l, b10l, b11l, c1l, Tl_tilde, Hl,
                                sc, loss_weights, epochs_4, lr_4, batch_perc=0.34)
    ...
    tmp = model([t_tilde, rt_tilde, St_tilde, kappa, sigma_r, sigma_S, rho, b0, b10, b11, c1, T_tilde])
    ...
    loss_tab = train_function(model, 
                                t_tilde,  rt_tilde, St_tilde, kappa,  sigma_r,  sigma_S,  rho,  b0,  b10,  b11,  c1,  T_tilde,
                                          rT_tilde, ST_tilde, kappaT, sigma_rT, sigma_ST, rhoT, b0T, b10T, b11T, c1T, TT_tilde, HT,
                                tl_tilde, rl_tilde, Sl_tilde, kappal, sigma_rl, sigma_Sl, rhol, b0l, b10l, b11l, c1l, Tl_tilde, Hl,
                                sc, loss_weights, 10, lr_4, batch_perc=0.34)
    train_time = 0
...
F_NN = model([t_tilde_test, rt_tilde_test, St_tilde_test, kappa_test, sigma_r_test, sigma_S_test, rho_test, b0_test, b10_test, b11_test, c1_test, T_tilde_test])

# fair value given by the closed-form formula
F_cf = F(t_test, T_test, rt_test, St_test, kappa_test, sigma_r_test, sigma_S_test, rho_test, K_test, b0_test, b10_test, b11_test, c1_test)
...
exists_resall = os.path.exists('EU_multi_results_all.csv')
if not exists_resall:
    with open('EU_multi_results_all.csv', 'w', newline='') as file:
         writer = csv.writer(file, delimiter=';')
         writer.writerow(["Configuration","Weighted Loss", "Inner Loss", "Maturity Loss", "Lower Loss", "Total Loss",
                          "MSE_alea_rt", "MSE_alea_St", "MSE_conf_rt", "MSE_conf_St", "MSE_alea" , 
                          "MSE_conf",  "relative error (in the money)", "Training time [sec]", "TRAIN"])
    ...
    with open('EU_multi_results_all.csv', 'a', newline='') as file:
         writer = csv.writer(file, delimiter=';')
         writer.writerow([Directory_name, round(loss_tab[:,0][-1], 6) , round(loss_tab[:,1][-1], 6),
                          round(loss_tab[:,2][-1], 6), round(loss_tab[:,3][-1], 6), round(loss_tab[:,4][-1], 6), round(MSE[0], 6),  
                          round(MSE[1], 6), round(MSE[2], 6), round(MSE[3], 6), round(MSE[4], 6), 
